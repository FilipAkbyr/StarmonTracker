...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $lcs = $_POST["LCS"];
    if (!is_numeric($lcs) || $lcs < 1 || $lcs > 255) {
        $errmsg .= "LCS, ";
        $throwerr = true;
    }
    ...
    $fcs = $_POST["FCS"];
    $isValid = false;
    // Check if the number is a valid decimal or hexadecimal
    if (preg_match('/^0x[0-9A-Fa-f]+$/i', $fcs) || preg_match('/^[0-9]+$/', $fcs)) {
        $isValid = true;
    }
    if (!$isValid || ($fcs < 1 && !preg_match('/^0x/', $fcs))) {
        $errmsg .= "FCS, ";
        $throwerr = true;
    }
    ...
    try {
        ...
        $stmt->bindParam(":FCS", $fcs, PDO::PARAM_STR);
